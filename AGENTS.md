...
- **Held Object Synchronization, Trajectory & Wall-Clearance Clamping**:
  ...
  - **Authoritative Grab & Optimistic Pickup Window (`recentlyPickedUpObjects`)**:
    - When a player grabs an object, the client immediately sends a dedicated `player_pickup` message to the server (`{ type: 'player_pickup', localId, objectId }`) and marks the object in `recentlyPickedUpObjects` with a 1500ms TTL.
    - **Glitch-Back Prevention**: In `reconcileWorldState`, incoming server snapshots where `sp.heldObjectId === null` (from before the server processed the pickup) are barred from resetting `lChar.heldObject` or snapping the object to the floor while `recentlyPickedUpObjects` is active.
    - **Server Verification**: `GameServer.js` handles `player_pickup` immediately via `executePickup`, validating reach and strength, setting `obj.isHeld = true`, `obj.heldBy = playerId`, and updating object hands position. Once the server confirms the attachment in `world_state`, the temporary optimistic lock is cleared.
  - **Authoritative Throw & Launch Verification**:
    - When a player throws an object, the client sends `player_throw` including ballistic launch velocities ($v_x, v_y, v_z$) and initial release coordinates (`startX`, `startY`, `startZ`).
    - **Server Power Validation**: `GameServer.js` verifies the player was holding or in reach of the object, validates throw power against character strength/recoil limits (speed ceiling $\le 25.0\text{ u/s}$), applies the release coordinates and velocities, clears `player.heldObjectId`, and flags `obj.lastThrower = playerId`.
    - **Self-Collision Repulsion Exclusion (Client & Server)**: In `ContinuousPhysics.resolveContinuousCollisions`, `GameLoop.resolveFreebodyCollisions`, and server TOI solver, collisions where `a.lastThrower === b || b.lastThrower === a` (or matching ID) are skipped while the object is departing within reach or safe collider separation. This prevents the thrower from colliding with the object upon release on frame 1.
    - **Dual-Loop Optimistic Release Lock & Stale Packet Filtering (`recentlyReleasedObjects`)**: Prevents in-flight lagging server packets (where `so.isHeld = true` before the server processed the throw) from overwriting ballistic flight velocities or snapping the object back into hands. Soft drift correction (> 1.2u) reconciles position smoothly if server and client diverge while flying, eliminating the 1-second drop-then-glitch bug.
  - **Throw Commitment Guarantee & Quick-Click Protection**:
    - Any press of the throw action (even an instantaneous 1ms click or tap of mouse left click, RT trigger, or RB bumper) commits the throw unconditionally.
    - **Client-Side Commitment (`throwCommitUntil`)**: Active for 450ms following throw initiation. Outgoing input packets maintain `isThrowHeld = true` and lock `isGrabHeld = false`, guaranteeing the server never interprets a quick button release as an intentional drop (`executeDrop`). `throwEvent` is retained and dispatched across multiple ticks until confirmed.
