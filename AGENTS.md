...
- **Held Object Synchronization, Trajectory & Wall-Clearance Clamping**:
  ...
  - **Authoritative Throw & Launch Verification**:
    ...
    - **Self-Collision Repulsion Exclusion**: In `resolveTOICollisions`, collisions where `a.lastThrower === b.id || b.lastThrower === a.id` are skipped while the object is departing. This eliminates the previous bug where the thrower's collider collided with the object in their hand upon release, killing forward momentum and dropping it right in front of the player.
    - **Dual-Loop Optimistic Release Lock (`recentlyReleasedObjects`)**: Prevents in-flight lagging server packets from overwriting ballistic flight velocities during the RTT window across both character hands and dynamic object loops.
