...
// Approach 1: Current code (sub-steps with break;)
// Approach 2: Commit 2fb7090 (single step with outwardVel subtraction)
// Approach 3: Tangent redirection with full speed

function testApproach2() {
  console.log("\n--- Approach 2 (Commit 2fb7090, no sub-steps) ---");
  ...
  for (let f = 1; f <= 25; f++) {
    ...
    if (fullSupport) {
      ...
      if (closest && closest.dist > 0) {
        ...
        const outwardVel = char.velocity.x * normalX + char.velocity.y * normalY;
        if (outwardVel > 0) {
          char.velocity.x -= outwardVel * normalX;
          char.velocity.y -= outwardVel * normalY;
        }

        const maxAllowedDist = r - 0.002;
        if (closest.dist > maxAllowedDist) {
          char.position.x = closest.closestX + normalX * maxAllowedDist;
          char.position.y = closest.closestY + normalY * maxAllowedDist;
        } else {
          char.position.x = candidateX;
          char.position.y = candidateY;
        }
...
function testApproach3FullSpeed() {
  console.log("\n--- Approach 3 (Tangent redirection at full speed) ---");
  const char = new Character({ x: 5.8, y: 4.1 });
  char.position.z = 1.0;
  char.supportingSurfaceHeight = 1.0;
  const neInput: Vector2D = { x: 1, y: -1 };
